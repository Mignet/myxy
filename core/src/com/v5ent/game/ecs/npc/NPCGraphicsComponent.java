...
public class NPCGraphicsComponent extends GraphicsComponent {
    ...
    public void update(Entity entity, MapManager mapMgr, Batch batch, float delta){
        ...
        if(_currentFrame!=null){
        	batch.begin();
        	batch.draw(_currentFrame, getCurrentPosition().x-_currentFrame.getRegionWidth()/2, getCurrentPosition().y, _currentFrame.getRegionWidth(), _currentFrame.getRegionHeight());
        	batch.end();
        }else{
        	Gdx.app.error(TAG, "not animation in "+_currentDirection);
        }
    ...
    private void drawSelected(Entity entity, MapManager mapMgr){
        ...
        float y = rect.y * Map.UNIT_SCALE - height/2+10;
