...
import com.v5ent.game.dialog.ConversationGraphObserver;
...
public class PlayerHUD implements Screen, AudioSubject, ProfileObserver,ComponentObserver,ConversationGraphObserver,StoreInventoryObserver, BattleObserver, InventoryObserver, StatusObserver {
    ...
    public PlayerHUD(Camera camera, Entity player, MapManager mapMgr) {
        ...
        _json = new Json();
        _messageBoxUI = new Dialog("消息", Utility.STATUSUI_SKIN, "solidbackground"){
            {
                button("确定");
                text(INVENTORY_FULL);
            }
        ...
        //地图名称
        _mapName = new Label("未知之地",Utility.STATUSUI_SKIN);
        _mapName.setPosition(0, _stage.getHeight()-20);
        _mapName.setVisible(true);
        
        _playerCood = new Label("0,0",Utility.STATUSUI_SKIN);
        _playerCood.setPosition(0, _stage.getHeight()-40);
        _playerCood.setVisible(true);
        
        _fps = new Label("FPS:0",Utility.STATUSUI_SKIN);
        _fps.setPosition(_stage.getWidth()-60, _stage.getHeight()-20);
        _fps.setVisible(true);
        
        _clock = new ClockActor("0", Utility.STATUSUI_SKIN);
        _clock.setPosition(_stage.getWidth()-_clock.getWidth()-20,0);//汉字差异
        ...
        _stage.addActor(_mapName);
        _stage.addActor(_playerCood);
        _stage.addActor(_fps);
        ...
        switch(event){
            case PROFILE_LOADED:
                ...
                if( firstTime ){
                    ...
                    _statusUI.setHPValue(hpVal==0?1:hpVal);
    ...
    public void render(float delta) {
        ...
        Object mapName = _mapMgr.getCurrentTiledMap().getProperties().get("mapName");
        _mapName.setText(mapName!=null?mapName.toString():"Unkonwn");
        Vector2 playerCood = _mapMgr.getPlayerPositionInMap();
        _playerCood.setText((int)playerCood.x+","+(int)playerCood.y);
        _fps.setText("FPS:"+Gdx.graphics.getFramesPerSecond());
        _stage.act(delta);
        _stage.draw();
    }
