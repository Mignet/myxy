...
public class NewGameScreen extends GameScreen {
	...
	public NewGameScreen(MyRpgGame game){
		...
		Label profileName = new Label("请输入存档文件名称: ", Utility.STATUSUI_SKIN);
		_profileText  = new TextField("",Utility.STATUSUI_SKIN, "inventory");
		_profileText.setMaxLength(20);

		_overwriteDialog = new Dialog("是否覆盖?", Utility.STATUSUI_SKIN, "solidbackground");
		Label overwriteLabel = new Label(" 覆盖已存在的存档?", Utility.STATUSUI_SKIN);
		TextButton cancelButton = new TextButton("取消", Utility.STATUSUI_SKIN, "inventory");

		TextButton overwriteButton = new TextButton("覆盖", Utility.STATUSUI_SKIN, "inventory");
