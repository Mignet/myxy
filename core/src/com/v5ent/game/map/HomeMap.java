...
public class HomeMap extends Map{
    ...
    private static String _mapPath = "maps/home.tmx";
    private Json _json;

    HomeMap(){
        super(MapFactory.MapType.HOME, _mapPath);
        Gdx.app.debug(TAG, "Loading HOME MAP:"+_mapPath);
        _json = new Json();

        if(_npcStartPositions!=null){
        ...
        Entity guanyin = EntityFactory.getInstance().getEntityByName(EntityFactory.EntityName.NPC_GUANYIN);
        initSpecialEntityPosition(guanyin);
        _mapEntities.add(guanyin);
